    ...
    @objc private func sceneTapped(recognizer: UITapGestureRecognizer) {
        ...
        if hitResults.count > 0 {
            ...
            if(name == "Man_Skin"){
                ...
                let path = "${PROJECT_DIR}/MPM-Sft-Eng/Assets.xcassets"
                let baseurl: URL = URL(fileURLWithPath: path)
               // enumerator(atPath: baseurl)?.forEach({ (e) in
...
//
//                let documentsURL = try fileManager.url(for: .documentDirectory, in: .Assets.xcassets, appropriateFor: nil, create: false)
